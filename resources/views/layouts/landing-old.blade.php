...
<div class="h-screen flex flex-col">
    @isset($notification)
    <div class="w-100 p-2 bg-brand-pastel border-b-2 border-brand text-brand font-medium md:text-center font-display flex flex-col items-center justify-center text-center">
    ...
    <nav class="px-2 sm:px-4 py-2.5 w-full z-20 top-0 left-0 border-b border-gray-200 font-display">
        <div class="container flex flex-wrap items-center mx-auto">
            ...
            <div class="flex md:order-2 ml-auto items-center">
                <a href="#" class="text-brand border-brand border-2 hover:bg-brand hover:text-white duration-300 transition-all font-bold rounded-lg text-sm px-5 py-2.5 text-center mr-6 tracking-tight hidden md:inline-block">{{ __('Donate') }}</a>
                <a href="{{ route('auth.signin.show') }}" class="text-white border-2 bg-brand border-brand hover:border-black hover:bg-black transition-colors duration-300 focus:outline-none font-bold rounded-full text-sm px-5 py-2.5 text-center mr-3 md:mr-0 tracking-tight hidden md:inline-block">{{ __('Sign In') }}</a>
            ...
            <div class="items-center justify-between hidden w-full md:flex md:w-auto md:order-1" id="navbar-items">
                <ul class="flex flex-col p-4 mt-4 border border-gray-100 rounded-lg bg-gray-100 font-bold md:flex-row md:space-x-8 md:mt-0 md:text-sm md:font-medium md:border-0 md:bg-transparent">
                    <li>
                        <a href="{{ route('index') }}" class="block py-2 pl-3 pr-4 md:font-semibold {{ Route::current()->getName() == 'index' ? 'text-brand' : 'text-white md:text-black md:hover:border-b-brand' }} md:border-b-2 md:border-transparent md:bg-transparent md:p-0">{{ __('Home') }}</a>
                    </li>
                    <li>
                        <a href="#" class="block py-2 pl-3 pr-4 transition-colors duration-300 md:font-semibold {{ Route::current()->getName() == 'host.index' ? 'text-brand' : 'md:text-black md:hover:border-b-brand' }} md:border-b-2 md:border-transparent md:bg-transparent md:p-0">{{ __('Host') }}</a>
                    </li>
                    <li>
                        <a href="#" class="block py-2 pl-3 pr-4 transition-colors duration-300 md:font-semibold {{ Route::current()->getName() == 'blog.index' ? 'text-brand' : 'md:text-black md:hover:border-b-brand' }} md:border-b-2 md:border-transparent md:bg-transparent md:p-0">{{ __('Blog') }}</a>
                    </li>
                    <li>
                        <a href="#" class="block py-2 pl-3 pr-4 transition-colors duration-300 md:font-semibold {{ Route::current()->getName() == 'store.index' ? 'text-brand' : 'md:text-black md:hover:border-b-brand' }} md:border-b-2 md:border-transparent md:bg-transparent md:p-0">{{ __('Shop') }}</a>
                    </li>
                </ul>
            </div>
        </div>
...
<footer class="mt-auto w-full flex flex-col p-10 bg-gray-100 font-display max-md:text-xl">
