...
<nav class="px-2 sm:px-4 py-2.5 w-full z-20 top-0 left-0 border-b border-gray-200 font-display">
    <div class="container flex flex-wrap items-center mx-auto">
        <a href="{{ route('index') }}" class="flex items-center md:mr-5">
            <img src="{{ mix('images/logo.svg') }}" class="h-10 mr-3 filter" alt="{{ __('Logo') }}">
        ...
        <div class="flex md:order-2 ml-auto items-center">
            ...
            @auth
            <a href="{{ route('app.channels.me') }}" class="text-white bg-brand hover:bg-black transition-colors duration-300 focus:ring-4 focus:outline-none focus:ring-brand/50 font-bold rounded-full text-sm px-5 py-2.5 text-center mr-3 md:mr-0 tracking-tight hidden md:inline-block">{{ __('Open App') }}</a>
            @else
            <a href="{{ route('auth.signin.show') }}" class="text-white bg-brand hover:bg-black transition-colors duration-300 focus:ring-4 focus:outline-none focus:ring-brand/50 font-bold rounded-full text-sm px-5 py-2.5 text-center mr-3 md:mr-0 tracking-tight hidden md:inline-block">{{ __('Sign In') }}</a>
            @endauth
        ...
        <div class="items-center justify-between hidden w-full md:flex md:w-auto md:order-1" id="navbar-items">
            <ul class="flex flex-col p-4 mt-4 border border-gray-100 rounded-lg bg-gray-50 md:flex-row md:space-x-8 md:mt-0 md:text-sm md:font-medium md:border-0 md:bg-transparent">
                ...
                <li>
                    <a href="#" class="block py-2 pl-3 pr-4 transition-colors duration-300 md:font-semibold {{ Route::current()->getName() == 'store.index' ? 'text-brand' : 'md:text-black md:hover:border-b-brand' }} md:border-b-2 md:border-transparent md:bg-transparent md:p-0">{{ __('Store') }}</a>
                </li>
            </ul>
        </div>
    </div>
